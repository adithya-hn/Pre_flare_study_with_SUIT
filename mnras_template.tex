...
\title[Short title, max. 45 characters]{Multi-Wavelength Diagnostics of Pre-Flare Evolution with Aditya-L1: From Chromosphere to Corona}

% The list of authors, and the short list which is used in the headers.
% If you need two or more lines of authors, add an extra line using \newauthor
\author[Adithya H. N.]{
Adithya H.N.,$^{1}$\thanks{E-mail: publications@ras.ac.uk (KTS)},
Sreejith Padinhatteeri $^{1}$,Soumya roy $^{1}$,
 Srikar P $^{3}$,
Durhesh Tripathi $^{2}$,
\newauthor
...
\begin{enumerate}
    ...
    \item How Chromosphere (Mg II and Ca II H) behaves during the pre-flare phase.
...
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{Figures/case8_lc.eps}  
    \caption{a) \ion{Mg}{ii} h image during the flare peak b)Light curve of \ion{Ca}{ii} H c) Light curve of HEL1OS (both CdTe 1 and CdTe 2 detectors, 9-40 keV), d) and E) Temperature and Emission measure derived from SoLEXS,  }
    \label{fig:enter-label}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{Figures/nb3_plot_time_stamps.pdf}
    \caption{SUIT time stamp images}
    \label{fig:placeholder}
\end{figure}

\subsection{Identifying pre-flare enhancements in SUIT}
To identify the pre-flare enhancements, we adopted the base-difference method. The base image was constructed using the median of the first five images in the 2-hour pre-flare sequence, which also minimises first-frame selection bias.
The base-difference images contain quiet-Sun and plage-related fluctuations, as well as intensity fluctuations caused by the contamination pattern. To mitigate these effects, we estimated the mean behaviour by computing the mean and standard deviation, which were affected by bright events. Therefore, we computed the median and median absolute deviation of the difference image, which are less affected by bright events. And we set a detection threshold of
 $median + 5 \times MAD \sigma$ was applied to the base-difference image, and contours were drawn around the regions exceeding this threshold.

Additionally, we imposed a size constraint of 16 pixels (corresponding to a PSF FWHM of 8 pixels). Only features larger than the PSF were retained. The base-difference total intensity within these selected regions was then used to generate the enhancement light curve."
...
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{Figures/c8_helios_aia_131_fd_roi_lc.png}
...
We observe a slight delay between HEL1OS and 131 light curves, most likely due to a difference in the energy bands of observation.
...
Currently, the HEL1OS counts are insufficient to fit the spectra, and with STIX spectra we are analysing pre-flare enhamcements.

\subsection{Hot onset cases}
In most of the flare cases, we observe elevated temperatures during flare onset in SoLEXS.  We also notice HEL1OS counts during that period. 
...
Luckily, we have an STIX observation for the last 5 cases; using STIX data, we analyse the hot onset part by fitting the thermal and non-thermal parts.

\section{Results}

\subsection{Case 4: M1.5 class flare on July 10}
We observe multiple brightenings in the pre-flare window (Fig. \ref{c4_lc}). All the brightening is occurring on the PIL, and some are located close to it. 

Several of the prominent enhancements exhibit corresponding pre-event signatures in HEL1OS. 
To confirm their source locations, we examined the AIA 131 Å images.
For three such events, we found a time delay of approximately 90 seconds between the HEL1OS signal and the \ion{Mg}{ii}~h response.
(DCF plot, HMI plot required)
The continuous enhancement began to appear at the flaring location after 05:28, which is approximately 31 minutes before the flare's soft X-ray peak time (based on the GOES data).
...
Pre-flare window we can observe shearing movement of positive and negative close to PIL.
This might have triggered the flare.

\subsection{Case 5: M1.1 class flare on July 10}
It is another flare from the same active region as above (AR 13878).
In this case also we observe multiple brightening in pre- flare window. Four major brightening location were identified including flare location (B1, B2, B3 and B4). 
All brightenings are on or close to PIL (Can not differentiate the exact location due to PSF size).

After 14:45 onwards continuous brightening is observed, B2 and B3 are constant contributors and B1 is pulsating, note that all three regions are connected.
SoLEXS temperature does not show much of variation fluctuates between 6-7 MK.

Flare might have triggered due too failed eruption or due to light bridge.

\subsection{Case 6: X1.8 class flare on October 9}
One of the strong flares in our dataset was produced by AR 13848.
Minor pre-flare activity was detected in this active region. A small brightening was observed at point 6P1, which may represent a small counterpart of the coronal activity seen in the AIA 171 Å images.

The flare ribbons displayed a flipped ‘S’ morphology, suggesting a sigmoidal flare structure, which is further supported by the XRT soft X-ray observations.
The flare may have been triggered by its own magnetic instability, which is a characteristic behaviour of sigmoidal flares.

\subsection{Case 7: M1.3 class flare on November 1}
...
\subsection{Case 8: M2.0 class flare on November 1}
...
\subsection{Case 9: M1.0 class flare on November 13}
...
\subsection{Case 10: M1.7 class flare on November 13}
...
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{Figures/case4_lc.eps} 
    \caption{Case 4 Light curves}
    \label{c4_lc}
\end{figure}
...
\begin{figure}
    ...
    \caption{Case 4 images }
    \label{fig:placeholder}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{Figures/c4_helios_aia_131_fd_roi_lc.eps}
    \caption{Case 4: AIA 131 light curve}
    \label{fig:placeholder}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{Figures/case5_lc.eps}
...
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{Figures/c5_helios_aia_131_fd_roi_lc.eps}
...
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{Figures/case6_lc.eps}
...
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{Figures/c6_helios_aia_131_fd_roi_lc.eps}
...
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{Figures/case7_lc.eps}
...
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{Figures/c7_helios_aia_131_fd_roi_lc.eps}
...
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{Figures/c8_helios_aia_131_fd_roi_lc.eps}
...
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{Figures/case9_lc.eps}
...
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{Figures/c9_helios_aia_131_fd_roi_lc.eps}
...
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{Figures/case10_lc.eps}
...
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{Figures/c10_helios_aia_131_fd_roi_lc.eps}
...
\begin{figure}  
    \centering
    \includegraphics[width=1 \linewidth]{Figures/case1_lc.png}
