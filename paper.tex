...
The base-difference images contain quiet-Sun and plage-related fluctuations, as well as intensity fluctuations caused by the contamination pattern. To mitigate these effects, we had to set a threshold to see some strong signals only. 
Mean and standard deviation of the image were affected by bright events.
Therefore, we computed the median and median absolute deviation (MAD) of the difference image, which are less affected by bright events. And we set a detection threshold of
...
\begin{equation}
Threshod= median +5 \times MAD ~ \sigma
\end{equation}
\noindent
where 
\begin{align*}
\tilde{X}=median(X_i)\\
MAD ~ \sigma = median (|X_i-\bar{X}|)\times 1.4826
\end{align*}
...
Several of the enhancements exhibit corresponding pre-event signatures in HEL1OS. 
